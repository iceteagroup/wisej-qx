  ...
  */
  ...
  members :
  {
    ...
    */
    ...
    // @ITG:Wisej: Add check to determine whether the FontManager is busy checking fonts.
    /**
     * Returns true if the FontManager is still busy validating fonts.
     */
    isValidating: function () {

      if (this.__queue && this.__queue.length > 0)
        return true;

      if (this.__queueInterval && this.__queueInterval.isEnabled())
        return true;

      for (var familyName in this.__validators) {
        if (this.__validators[familyName].isValidating())
          return true;
      }
    ...
    */
    ...
    __getRule : function(familyName, sourcesMap)
    {
      ...
      // @ITG:Wisej: Validate the sources.
      if (rules.length == 0)
        return null;
