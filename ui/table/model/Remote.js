...
qx.Class.define("qx.ui.table.model.Remote",
{
  ...
  */
  ...
  members :
  {
    ...
    {
      // @ITG:Wisej: Check the rowIndex or we may remove the last row when removing an index out of bounds.
      ...
      else
      {
        ...
        for (var block=startBlock; block<=blockCount; block++)
        {
          ...
          if (blockData != null)
          {
            ...
            //       (This is the row that was moved up)
            var removeIndex = 0;
    ...
    insertRow: function (rowIndex, rowData) {
        ...
        // Insert the row and move the rows of all following blocks
        for (var block = startBlock; block <= blockCount; block++) {
            ...
            if (blockData != null) {
                ...
                // Insert the row in the start block
                // and move the last row down to the next blocks.
                if (block == startBlock) {

                    var insertIndex = rowIndex - block * blockSize;
                    blockData.rowDataArr.splice(insertIndex, 0, rowData);
                }


                // move the last row of this block to the top of the next block.
                var nextBlockData = this._rowBlockCache[block + 1];
    ...
    // @ITG:Wisej: Added setRowData.

    /**
     * Inserts a row into the model.
     *
     * @param rowIndex {Integer} the index of the row to update.
     * @param rowData {Map} the new values of each column.
     */
