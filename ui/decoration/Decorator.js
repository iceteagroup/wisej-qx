...
qx.Class.define("qx.ui.decoration.Decorator", {
  ...
  include : [
    ...
    // @ITG:Wisej: Added transform decorator.
    qx.ui.decoration.MTransform,
    // @ITG:Wisej: Added custom stylesheet decorator.
    qx.ui.decoration.MStyleSheet,
  ],
  ...
  members :
  {
    ...
    // @ITG:Wisej: Moved the  local directions out to a field.
    ...
    _getDefaultInsets : function() {
      ...
      // @ITG:Wisej: Cached the "_getDefaultInsetsFor" functions instead of scanning all fields all the times.
      var methods = this.__getDefaultInsetsMethods();
      for (var m = 0; m < methods.length; m++) {

        var currentInsets = methods[m].call(this);

        for (var i = 0; i < this.__directions.length; i++) {
            var direction = this.__directions[i];
    ...
    __defaultInsetsMethods: null,
    __getDefaultInsetsMethods: function () {

        if (this.__defaultInsetsMethods)
            return this.__defaultInsetsMethods;

        this.__defaultInsetsMethods = [];
        for (var name in this) {

            if (name.indexOf("_getDefaultInsetsFor") == 0 && this[name] instanceof Function)
                this.__defaultInsetsMethods.push(this[name]);
        }
        return this.__defaultInsetsMethods;
    },
