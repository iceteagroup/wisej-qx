  ...
  members :
  {
    ...
    */
    ...
    add : function(obj)
    {
      ...
      reg.push(obj);
      // @ITG:Wisej: Menus should have a z-index higher than popups. Changed 1e6 to 1e7 here and 1e7 to 1e6 in the popup manager.
      obj.setZIndex(1e7 + reg.length);
    },
    ...
    */
    ...
    _onPointerDown : function(e)
    {
      ...
      // If the target is 'null' the tap appears on a DOM element which is not
      // a widget. This happens normally with an inline application, when the user
      // taps not in the inline application. In this case all currently
      ...
      // @ITG:Wisej: Added check for isVisible, otherwise we get js errors when clicking on
      // widgets like qx.ui.embed.Flash() that have a menu property.
      if (target.getMenu) {
        var menu = target.getMenu();
    ...
    */
    ...
    _onRoll : function(e) {
      ...
      if (
        ...
        // @ITG:Wisej: Ignore roll events without a delta, or we close the popup when shifting the focus and clicking at the same time.
        var delta = e.getDelta();
        if (delta && (delta.x + delta.y) == 0)
            return;
