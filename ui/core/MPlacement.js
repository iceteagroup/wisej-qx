  ...
  members :
  {
    ...
    moveTo : function(left, top)
    {
      ...
      if (visible) {
        ...
        if (bounds && elemLocation) {
          ...
          // each number is for the upcoming check (huge element is
          // the always visible, element prefixed)
          //     | 3 |
          //   ---------
          //   | |---| |
          //   |       |
    ...
    placeToWidget : function(target, liveupdate)
    {
      ...
      if (liveupdate)
      {
        ...
        // Bind target and live update to placeToWidget
    ...
    _place : function(coords)
    {
      this.__getPlacementSize(function(size)
      {
        ...
        // @ITG:Wisej: Verify that the widget has a layout parent
        // or we may get a javascript error when the page is unloading
        // and an automatically placed widget tries to line up with the parent.
        ...
        // @ITG:Wisej:Added states for above and below placements.
        ...
        if (bounds) {
          ...
          result.right = result.left + bounds.width;
          result.bottom = result.top + bounds.height;
        }
