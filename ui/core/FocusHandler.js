...
qx.Class.define("qx.ui.core.FocusHandler",
{
  ...
  */
  ...
  members :
  {
    ...
    /**
     * Moves the focus to the next widget in the tabbing order.
     *
     * @param widget {qx.ui.core.Widget ? null} The starting widget. If null, it will use the currently focused widget, or start with the first.
     */
    focusNext: function(widget)
    {
        var current = widget || this.getFocusedWidget();
    ...
    /**
     * Moves the focus to the previous widgets in the tabbing order.
     *
     * @param widget {qx.ui.core.Widget ? null} The starting widget. If null, it will use the currently focused widget, or start with the last.
     */
    focusPrev: function (widget) {

        var current = widget || this.getFocusedWidget();
        var prev = current ? this.__getWidgetBefore(current) : this.__getLastWidget();
        if (prev)
            prev.tabFocus();
    ...
    */
    ...
    {
      // @ITG:Wisej: getLayoutChildren() is wrong in this context. It excludes the child controls with user bounds.
      // var children = parent.getLayoutChildren();
      var children = this.__getChildren(parent);
      var child;
    ...
    __getLast : function(parent, lastWidget)
    {
  	...
  	/**
	 * Returns the list of children or an empty array.
	 */
    __getChildren: function (parent) {

        return parent.getChildren
            ? parent.getChildren()
            : parent.__emptyChildren;
