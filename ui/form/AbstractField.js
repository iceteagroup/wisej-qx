  ...
  members :
  {
    ...
    */
    ...
    setLayoutParent : function(parent)
    {
      ...
      if (this.__useQxPlaceholder) {
        if (parent) {
          // @ITG:Wisej: If the textfield is added again to the same container
          // this call generates an exception when QX is using placeholders for the watermark instead of css.
          // this.getLayoutParent().getContentElement().add(this._getPlaceholderElement());
          var placeholder = this._getPlaceholderElement();
          var el = this.getLayoutParent().getContentElement();
          if (el.indexOf(placeholder) == -1)
            el.add(placeholder);
          } else {
          var placeholder = this._getPlaceholderElement();
    ...
    _onPointerDownPlaceholder : function() {
      window.setTimeout(function() {
        ...
        // @ITG:Wisej: This causes an error when focusing "inner" textfield children that are not focusable..
