  ...
  members :
  {
    ...
    */
    ...
    _onPointerDown : function(e)
    {
      ...
      // @ITG:Wisej: make sure the knob has been created before reading the location.
      var knobLocation = 0;
      var knobEl = knob.getContentElement().getDomElement();
      if (knobEl)
        knobLocation = this.__knobLocation = qx.bom.element.Location.get(knobEl)[locationProperty];
    ...
    */
    ...
    _updateKnobSize : function()
    {
      ...
      // @ITG:Wisej: Enforce minWidth.
      var knob = this.getChildControl("knob");

      // Read size property
      if (this.__isHorizontal) {
        // @ITG:Wisej: Enforce minWidth.
        var width = Math.max(knobFactor * avail.width, knob.getMinWidth());
        knob.setWidth(width);
      } else {
        // @ITG:Wisej: Enforce minHeight.
        var height = Math.max(knobFactor * avail.height, knob.getMinHeight());
        knob.setHeight(height);
      }
    },
