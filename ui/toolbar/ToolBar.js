...
qx.Class.define("qx.ui.toolbar.ToolBar",
{
  ...
  */
    ...
    _recalculateOverflow : function(width, requiredWidth)
    {
     ...
     if (width < requiredWidth) {
       do {
         ...
         // new width is the requiredWidth - the removed children width
       ...
       do {
         ...
         if (removedChild) {
           ...
           // check if it fits in case its the last child to replace
           var fits = false;
  ...
  destruct : function() {
    ...
    // @ITG:Wisej: Dispose the overflow indicator or it leaks.
    if (!qx.core.ObjectRegistry.inShutDown) {
        var overflowWidget = this.getOverflowIndicator();
        if (overflowWidget)
            overflowWidget.destroy();
    }
  }
});
