  ...
  */
  ...
  members :
  {
    ...
    */
    ...
    // @ITG:Wisej: Added clearActions() to remove all supported actions. It allows
    // a widget to change the supported actions while processing the "drag".
    /**
     * Used during <code>drag</code> listener to
     * clear the list of supported actions.
     */
    ...
    // @ITG:Wisej: Added support for setCurrentAction() to allow the code to change the drop effect.
    /**
     * Sets the currently selected drop action.
     *
     * @param action {String} Any valid action
     * @return {String} One of <code>move</code>, <code>copy</code> or
     *    <code>alias</code>
     */
    ...
    */
    ...
    __detectAction : function()
    {
      ...
      if (this.__validDrop)
      {
        // @ITG:Wisej: Added support for setCurrentAction() to allow the target
        // to display the current action if it matched one of the allowed actions.
        if (this.__currentActionSet)
          return this.__currentAction;
    ...
    _onPointermove : function(e) {
      ...
      if (el !== cursorEl) {
        ...
        if (droppable && droppable != this.__dropTarget) {
          ...
          // @ITG:Wisej: Changed dragover to dragenter to keep compatibility with HTML5 drag-drop specs.
          // this.__validDrop = this.__fireEvent("dragover", droppable, this.__dragTarget, true, e);
          this.__validDrop = this.__fireEvent("dragenter", droppable, this.__dragTarget, true, e);

          // @ITG:Wisej: Added support for "drop" state.
          var dropTargetWidget = qx.ui.core.Widget.getWidgetByElement(this.__dropTarget);
          if (dropTargetWidget)
            dropTargetWidget.addState("drop");
        }

        // @ITG:Wisej: Added support frm "dragover" consistent with html5 drag-drop.
        else if (droppable && droppable == this.__dropTarget) {
